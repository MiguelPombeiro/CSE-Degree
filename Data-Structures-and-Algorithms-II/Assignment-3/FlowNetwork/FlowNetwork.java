...
public class FlowNetwork {
    ...
    public int edmondsKarp(){
        ...
        // for (int i = 0; i < nodes; ++i){
        //     // Updates the flow values in the original network
        //     for (Edge e : adjacents[i]){
        //         // System.out.println(i + " -> " + e.toString());
        //         System.out.println(i + " -> " + e.getDestination() + " Flow: " + e.getFlow() + "/" + e.getCapacity());
        //     }
        // }

        // for (Edge e: this.adjacents[source]){
        //     if (e.getFlow() < 1)
        //         System.out.println(e.getDestination() + " Does not get a card");
        // }

        for (int i = 0; i < nodes; i++) {
            if (i == source) continue; // Skip source and sink
            for (Edge e : adjacents[i]) {
                if (e.getFlow() > 0 && e.getDestination() != sink) {
                    System.out.println("Participant " + i + " exchanges card with participant " + e.getDestination());
                }
            }
        }
