...
plano_detalhado(PlanoLinear, Passos, Ordem, Links) :-
    plano_ini(Pi),
    pop(Pi, Pf),
    Pf = p(Passos, OrdemInv, LinksInv),
    reverse(OrdemInv, Ordem),
    reverse(LinksInv, Links),
    lineariza(Pf, PlanoLinear).
...
escolheSk(p(Passos,Ordem,Links),S,C,
        p(Passos,
          [m(Sk,S)|Ordem],
          [link(Sk,C,S)|Links])):-
...
[m(Sk,S),m(Sk,s2),m(s1,Sk)|Ordem],[link(Sk,C,S)|Links])) :-
    length(Passos,M),
    M < 6,
    I is M + 1,
    name(I,L),
...
%S3 ameaca o link(S1,C,S2) se S3 tem C na delete list e S1<S3<S2
ameaca(S1,S2,S3,Ord):- 
    consistente([m(S1,S3),m(S3,S2)|Ord]).
...
lineariza(p(Passos,Ordem,_Links),P):-
    ...
    length(Nomes,N), member(n(X,s2),Nomes), 
    X#>=0, 
    X #=< N,
...
variaveis([n(X,_)|R],[X|S]):- 
    variaveis(R,S).


plano_ord([],_,[]).

plano_ord([n(_,S)|R],Passos,[S-N|Passos2]):- 
    member(S-op(N,_,_,_),Passos),
